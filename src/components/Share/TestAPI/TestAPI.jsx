...
import classes from "./TestAPI.module.css";
const TestAPI = ({
  maxItemCount,
  anime,
  starIcons,
  ...
  const homeContainer = [classes.homeContainer];
  const homeItem = [classes.homeItem];
  const homeItemShortDiscription = [classes.homeItemShortDiscription];

  console.log(extraClasses);

  if (extraClasses) {
    homeContainer.push(classes.homeContainerProfile);
    homeItem.push(classes.homeItemProfile);
    homeItemShortDiscription.push(classes.homeItemShortDiscriptionProfile);
  }
  const hc = {
    padding: padding,
  ...
  if (filteredAnime.length !== 0) {
    return (
      <div className={homeContainer.join(" ")} style={hc}>
        {filteredAnime.map((item) => {
          return (
            <Link
              to={`/anime/${item.mal_id}`}
              className={homeItem.join(" ")}
              key={item.mal_id}
              onClick={() => window.scrollTo(0, 0)}
            >
              <img src={item.images.jpg.large_image_url} style={hiimg} />
              <div className={homeItemShortDiscription.join(" ")}>
                <h4 style={hih4}>
              ...
              {starIcons ? <StarIcons anime={item} size={size} /> : null}
    ...
    return (
      <h5 style={{ marginTop: "30vh", textAlign: "center" }}>Тут пока пусто</h5>
    );
  }
};
