...
const AnimeIdPage = () => {
  ...
  const [fetchAnimeById, isAnimeLoadingById, animeErrorById] = useFetching(
    async (id) => {
      const responce = await TestService.getAnimeById(id);
      const responceStatistic = await TestService.getAnimeStatistic(id);
      const responcePictures = await TestService.getAnimePictures(id);
      ...
      <div className="App">
        ...
        <div className="anime-page">
          ...
          <div className="discription">
            <h5>{`Япония, ${getSeason(animeById.data.season).toLowerCase()} ${
              animeById.data.aired.prop.from.year
            } г.`}</h5>
            <h5>{`${animeById.data.episodes} эп. по ~${getWithoutLetters(
              animeById.data.duration
            )} мин.`}</h5>
            <h5>{`Студия ${animeById.data.studios[0].name}`}</h5>
            <h5>
              {animeById.data.genres
          ...
          <div className="buttons">
            {user ? (
              <MyButton onClick={() => setVisible(true)}>
                {userDb?.anime
                  ? userDb.anime[animeById.data.mal_id]?.status || "Не смотрю"
                  : "Не смотрю"}
                <svg
